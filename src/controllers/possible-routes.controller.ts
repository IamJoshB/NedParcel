...
/**
 * @swagger
 * /api/possible-routes/link-driver:
 *   post:
 *     summary: Link a driver to an entity
 *     description: Links a driver to a taxi rank, banking details record, or association.
 *     tags: [Routes]
 *     operationId: linkDriverEntity
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 *           schema:
 *             $ref: '#/components/schemas/LinkDriverEntityRequest'
 *     responses:
 *       200:
 *         description: Driver updated with linked entity
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *               $ref: '#/components/schemas/ErrorResponse'
 */
export const linkDriver = async (req: Request, res: Response) => {
  const { driverId, entityType, entityId } = req.body;

  const validEntities = ["taxiRank", "bankingDetails", "taxiAssociation"];
  ...
  try {
    const updatedDriver = await Driver.findByIdAndUpdate(
      ...
      { [entityType]: entityId },
      { new: true }
    ).populate("taxiRank bankingDetails taxiAssociation");
    ...
    res.status(500).json({ message: "Error linking entity to driver", error });
  }
};

/**
 * @swagger
 * /api/possible-routes/unlink-driver:
 *   post:
 *     summary: Unlink a driver from an entity
 *     description: Removes a previously linked entity from a driver.
 *     tags: [Routes]
 *     operationId: unlinkDriverEntity
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 *           schema:
 *             $ref: '#/components/schemas/UnlinkDriverEntityRequest'
 *     responses:
 *       200:
 *         description: Driver updated with entity removed
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *               $ref: '#/components/schemas/ErrorResponse'
 */
export const unlinkDriver = async (req: Request, res: Response) => {
  const { driverId, entityType } = req.body;
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Error unlinking entity from driver", error });
  }
};
